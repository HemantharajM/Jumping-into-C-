...
bool balanced(Node *p_root) 
{
        if(p_root == NULL) {
                return true;
        } else {
                return count(p_root->p_left) == count(p_root->p_right) and balanced(p_root->p_left) and balanced(p_root->p_right);
        }
}
...
int main()
{
        ...
        cout <<"Is tree is balanced " << balanced(p_root) << endl;
}
